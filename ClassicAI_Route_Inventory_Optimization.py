...
# Classic AI Demo: Route & Inventory Optimization
...
min_threshold = 50
max_threshold = 150
sku = 'Milk'  # You can change to 'Cereal' or 'Soda'
    ...
    if res.success:
        ...
        for i, s_idx in enumerate(surplus_indices):
            for j, d_idx in enumerate(deficit_indices):
                qty = x[var_idx]
                if qty > 1e-2:
                    print(f"Transfer {qty:.1f} units from {store_names[s_idx]} to {store_names[d_idx]}")
                var_idx += 1
    else:
