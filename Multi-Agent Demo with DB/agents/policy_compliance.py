...
Tools:
  ...
  • query_memory         — find precedents for policy breaches (via MCP server)
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "query_memory",
            "description": (
                "Search memory for precedents — cases where a similar breach "
                "was resolved or accepted. Use to strengthen policy arguments."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
2. query_memory (vendor_history only — 1 call max)
3. check_policy_rule — HIGHEST RISK AREAS ONLY: liability cap, payment terms,
   data usage rights, SLA. Maximum 4 calls. Skip rules where contract clearly complies.
4. challenge_risk_agent — MANDATORY if any conflict found (see below)
5. issue_verdict — after all checks
...
MEMORY RULE: When memory returns a precedent where a similar breach
was resolved, cite it in your finding. One memory query only.
...
2. query_memory (vendor_history only — 1 call max)
3. check_policy_rule — HIGHEST RISK AREAS ONLY: liability cap, payment terms,
   data usage rights, SLA. Maximum 4 calls.
...
MEMORY RULE: When memory returns a precedent, cite it in your finding.
...
def handle_tool(name, args):
    if name == "read_contract":
        ...
        try:
            if qtype == "vendor_history":
                ...
                results = json.loads(_call_mcp("query_vendor_history",
                                               {"vendor_name": vendor}))
                if not results:
                    return f"No prior history with '{vendor}'."
                ...
                lines = [f"Precedent: {len(results)} similar {ctype} clause(s):"]
                for r in results[:3]:
                    m = r.get("metadata", {})
                    lines.append(f"  • {m.get('vendor','?')}: {r['document'][:80]}")
                memory_hit(f"Policy precedent: {lines[0]}")
                return "\n".join(lines)
                ...
                if not results:
                    return "No precedent found."
                return f"Found {len(results)} precedent(s)."
        ...
        # In standalone mode — completely silent, no output, no queue
        if not _state.get("is_collab"):
            return SILENT
        # In collab mode — queue the challenge for dispatch after tool loop
        challenge_data = {
...
def display_output(result):
    ...
    if breaches:
        ...
        for b in breaches:
            ...
            if b.get("memory_precedent"):
                memory_hit(f"Precedent: {b['memory_precedent'][:90]}")
    ...
    if result.get("memory_influenced_verdict"):
        print(f"\n  🧠 {result['memory_influenced_verdict']}")
...
def summary_for_controller(result):
    ...
    if result.get("memory_influenced_verdict"):
        lines.append(f"\n  🧠  {result['memory_influenced_verdict'][:100]}")
    return "\n".join(lines)


def run_as_service():
    header("AGENT 3 — POLICY & COMPLIANCE",
           "Tools: read_contract | query_memory | check_policy_rule | "
        ...
        for msg_type in ("standalone_request", "collab_request"):
            ...
            for filepath, msg in msgs:
                ...
                if _state["challenges_sent"] and is_collab:
                    ...
                    # Wait for a response for each challenge sent
                    for _ in _state["challenges_sent"]:
                        try:
                            ...
                            if decision == "upgrade":
                                ...
                                result["memory_influenced_verdict"] = (
                                    result.get("memory_influenced_verdict", "") +
                                    f" Agent 2 conceded on {rule} — "
                                    f"rating upgraded to RED."
                                ).strip()
...
if __name__ == "__main__":
    if "--service" in sys.argv:
        ...
        log("Tools: read_contract | query_memory | check_policy_rule | "
            "challenge_risk_agent | issue_verdict\n")
